...
async function loadInfrastructures() {
  const rawInfrastructureData = await db.collection('_global_infrastructures').find().toArray();
  if (rawInfrastructureData.length > 0) {
    return rawInfrastructureData[0].infraTypes;
  } else {
...
async function obtenerInfraestructuras(req, res) {
  try {
    ...
    res.status(500).json({ message: 'No se pudieron recuperar las infraestructuras' });
  }
}
...
async function obtenerCatalogoInfraestructuras(req, res) {
  try {
    ...
    res.status(500).json({ message: 'No se pudieron recuperar las infraestructuras del catálogo' });
  }
}
...
async function guardarInfraestructurasData(infraTypes) {
  // Actualiza los datos en MongoDB
  try {
    const mainDocumentId = await loadMainDocumentId();
    const filter = { _id: mainDocumentId };
    const update = { $set: { infraTypes: infraTypes } };
    await db.collection('_global_infrastructures').updateOne(filter, update);
  } catch (error) {
    console.error('Error al guardar los datos de infraestructura en MongoDB', error);
  }
}
