...
// // Función para cargar el JSON principal de infraestructuras
// function cargarInfraestructuras() {
//   const filePath = path.join(__dirname, 'jsons/_global_infrastructures.json');
...
// // Función para cargar el JSON del catálogo de infraestructuras
// function cargarCatalogoInfraestructuras() {
//   const filePath = path.join(__dirname, 'jsons/_global_cat_infrastructures.json');
//   if (fs.existsSync(filePath)) {
//     const rawData = fs.readFileSync(filePath);
//     return JSON.parse(rawData).infraTypes;
//   } else {
...
// function guardarInfraestructurasData(infraTypes) {
//   const filePath = path.join(__dirname, 'jsons/_global_infrastructures.json');
//   fs.writeFileSync(filePath, JSON.stringify({ infraTypes }, null, 4));
// }
...
//     }
//   } catch (err) {
//     res.status(500).json({ code: "ERROR", message: "Error retrieving infrastructures", error: err });
//   }
// }
...
//     }
//   } catch (error) {
//     res.status(500).json({ code: "ERROR", message: "Error retrieving infrastructure catalogue", error: error });
//   }
// }
...
//     res.status(200).json({ code: "OK", object: infrastructures, message: "Infraestructura agregada con éxito." });
//   } catch (error) {
//     res.status(500).json({ code: "ERROR", message: "No se pudo agregar la infraestructura", error: error });
//   }
// }
...
//     const regionId = req.query.regionId;
//     if (!(infrastructureId && parentId && regionId)) {
//       res.status(400).json({ code: "ERROR", message: "Parámetros de consulta obligatorios ausentes (debe tener _id && parentId && regionId)" });
//     } else {
//       const mainDocumentId = await loadMainDocumentId();
//       const query = {
//         _id: mainDocumentId,
...
//     }
//   } catch (err) {
//     res.status(500).json({ code: "ERROR", message: "Infraestructura no recuperada", error: err });
//   }
// }
...
//   const rawInfrastructuresData = await db.collection('_global_infrastructures').find().toArray();
//   const infrastructureFirstIndex = 0;
//   return rawInfrastructuresData[infrastructureFirstIndex]._id;
// }
...
//     res.status(200).json({ code: "OK", object: { _id: infraId, isEnabled: isEnabled, parent_id: parentId }, message: "Estado de la infraestructura actualizado con éxito." });
//   } catch (error) {
//     res.status(500).json({ code: "ERROR", message: "No se pudo actualizar el estado de la infraestructura", error: error });
//   }
// }
...
//   toggleInfrastructureStatus,
//   fetchInfrastructureById,
//   cargarCatalogoInfraestructuras,
//   cargarInfraestructuras,
//   guardarInfraestructurasData,
